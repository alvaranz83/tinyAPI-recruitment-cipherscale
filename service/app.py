...
class CreateFirstTechInterviewScoringRequest(BaseModel):
    positionId: str
    candidateName: str
    roleName: str
    content: Optional[str] = None
...
@app.post("/candidates/createFirstTechnicalInterviewScoringModel")
def create_first_tech_interview_scoring(request: Request, body: CreateFirstTechInterviewScoringRequest):
    require_api_key(request)
    subject = body.userEmail or _extract_subject_from_request(request)
    _, drive, docs = get_clients(subject)
    ...
    query = (
        ...
        "and trashed=false "
        f"and name='{doc_name}' "
        f"and '{scoring_folder_id}' in parents"
    )
    results = drive.files().list(
        q=query,
        fields="files(id,name)",
    ...
    if results.get("files"):
        existing = results["files"][0]
        return {
            "message": f"1st Technical Interview Scoring Rubric already exists for {body.candidateName}",
            "fileId": existing["id"],
            "folderId": scoring_folder_id,
            "docLink": f"https://docs.google.com/document/d/{existing['id']}/edit",
            "created": False
    ...
    content = body.content or f"""
        1st Technical Interview Scoring Rubric – {body.candidateName} ({body.roleName})
        ...
        Evaluation Categories:
        - Role Expertise (1–5): Depth of knowledge relevant to {body.roleName}.
        - Problem-Solving (1–5): Ability to analyze challenges and propose solutions.
        - Communication (1–5): Clarity, articulation, and ability to collaborate effectively.
        - Culture Fit (1–5): Alignment with company values, adaptability, and teamwork.
        - Leadership & Initiative (1–5): Ability to inspire, mentor, and take ownership.
    ...
    # ✅ Create the doc in Scoring Rubrics
    file_id = create_google_doc(docs, drive, scoring_folder_id, doc_name, content)

    return {
        "message": f"1st Technical Interview Scoring Rubric created for {body.candidateName}",
        "fileId": file_id,
        "folderId": scoring_folder_id,
        "docLink": f"https://docs.google.com/document/d/{file_id}/edit",
        "created": True
